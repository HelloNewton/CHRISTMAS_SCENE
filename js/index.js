...
MAIN_CONTAINER.appendChild(TREE_CONTAINER);
    ...
    const createSnow= (density)=>{
        for(let i = 0; i<density; i++){
            ...
            const RANDOM_FALL_SIZE =  `${getRandomValue (7,1)}px`;
            const RANDOM_FALL_OPACITY = Math.random().toFixed(2);   
            SNOW_FLAKE.classList.add("snow");
            SNOW_FLAKE.style.opacity = RANDOM_FALL_OPACITY
            SNOW_FLAKE.style.width = RANDOM_FALL_SIZE;
            SNOW_FLAKE.style.height = RANDOM_FALL_SIZE;
